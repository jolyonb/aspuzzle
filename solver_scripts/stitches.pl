...
adjoining_region(Id_1, Id_2) :- region(A, Id_1), region(B, Id_2), orthogonal(A, B), Id_1 < Id_2.

% Create stitches between adjoining regions
{ stitch(A, B) : region(A, Id_1), region(B, Id_2), orthogonal(A, B) } = stitch_count :- adjoining_region(Id_1, Id_2).
...
:- cell_in_stitch(A), N = #count{ cell(R, C) : stitch(A, cell(R, C)); cell(R, C) : stitch(cell(R, C), A) }, N != 1.

% Count stitches in each major line
:- expected_count(Dir, N, Counter), #count{ cell(R, C) : cell_in_stitch(cell(R, C)), line(Dir, N, cell(R, C)) } != Counter.

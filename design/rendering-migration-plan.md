...
The solution-dict contract keys atoms by bare predicate name (`Solver.solve` buckets on `type(atom).get_name()`), which is ambiguous for same-name/different-arity predicates and for classical negation (`-p`) — both legal in ASP, neither used by any current solver. The rendering layer is defended in the meantime: rules accept `type[Predicate] | str` (`PredicateRef`), and class references isinstance-filter their bucket, so a class-keyed rule stays correct even against a mixed bucket.

This step closes the hole at the source: rekey solution dicts by full signature (name/arity plus a negation marker), which is a **data-format change** — it touches `Solver.solve()`, `validate_solutions`, and the expected-solutions blocks of every `puzzles/*.json`, plus the string-reference convention in spec rules. Do it only when triggered: the first solver (or aspalchemy feature) that puts multi-arity or classically negated predicates into shown output. Until then the `PredicateRef` filter plus unique-name convention carries.

Also revisit at this step whether string references can be retired entirely. They exist because SymbolSet/RegionConstructor predicate classes are `construct_puzzle` locals and `get_render_spec` runs pre-construction (the preview path); making rules class-only would need stash-on-self across ~8 solvers plus construction-on-preview. With all ports done, the actual class/string ratio and the lifecycle appetite will be visible.
...
- **Hexagonal grid (the next grid; triangular is deferred):** new `aspuzzle/grids/hexgrid.py` — subclass `Grid`, implement the existing ASP abstract surface with axial coordinates (uniform vector per direction, so the base `Direction`/`Orthogonal` machinery works unchanged) plus the Step 3 topology methods (`neighbor`, `corner_names`, `corner_across`) and `ascii_geometry()` returning a new `HexAsciiGeometry` (in `aspuzzle/rendering/grids/hex_ascii.py`; design §5.4). Orientation is a pair of concrete sibling subclasses under the abstract base (`HexGrid` → `FlatTopHexGrid`/`PointyTopHexGrid`), each overriding the direction/corner vocabulary and geometry factory — design §5.6; the framework is orientation-blind. Two pre-flight items recorded in the design's §11: the config loader derives module names from class names, so orientation subclasses sharing `hexgrid.py` need the registry decision first; and `Solver.supported_grid_types` is annotated as a 1-tuple (`tuple[type]`) — widen to `tuple[type[Grid], ...]` before any solver declares two grid types. Parametrize the ASP-skew conformance test (`test_neighbor_matches_asp_orthogonal_facts`) over `ALL_GRID_FACTORIES` when the factory list grows — offset-coordinate neighbor arithmetic is exactly what it exists to catch.

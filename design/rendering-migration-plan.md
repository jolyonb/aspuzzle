...
**Create:**
- `aspuzzle/rendering/spec.py` — `RenderContext`, `Colorer`, `GlyphRule`, `FillRule`, `PathRule`, `EdgeRule`, `LinkRule`, `FromPredicate`, `FromClues`, `RegionFillRule`, `RegionBorderRule`, `RegionBoundaryRule`, `CustomRule`, `AtomRule`, `LineLabels`, `RenderSpec` (incl. `style` + per-backend `styles`, design §3.7), and the free function `build_scene(grid, spec, grid_data, solution)` — clue styles first (paint-below, matching `_build_render_grid`'s order), then atom rules (data-driven rules run with `solution=None`), then labels; provenance and `backends` stamped mechanically; **eager field-name validation with precise errors**.

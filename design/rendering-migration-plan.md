...
- `aspuzzle/rendering/scene.py` — `Provenance`, `Layer`, `EdgeWeight`, `Edge`, `Vertex`, `SceneElementBase` (`KW_ONLY` backends/provenance), the seven element dataclasses, `SceneElement` union, `CellStyle`, `SceneStyle`, `LayoutNeeds`, `Scene` (`add`/`extend`/`glyph`/`fill`/`line_labels`/`visible`/`sorted_elements`/`layout_needs`).
...
2. **`LayoutNeeds` is defined in `scene.py`** (pure data — edge/vertex sets + margin widths, no character knowledge), which is what lets `Scene.layout_needs(backend)` exist without violating the "scene/spec import nothing from `ascii/`" rule.
...
**Modify `aspuzzle/grids/base.py`** (`Grid`): add `cell_coords`, abstract `neighbor`, abstract `corner_names` (property), abstract `corner_across(corner, direction)` (the per-grid corner-reflection fact driving the generic vertex walk — design §3.4; cell-free signatures, per the triangular deferral), abstract `all_cells`, `opposite_direction(d)` helper (from `opposite_directions`), and `edge()` / `vertex()` canonical constructors (concrete in the base class: `edge` flips through the neighbor and keeps the lexicographically smaller spelling; `vertex` BFS-walks `corner_across` collecting every spelling and keeps the smallest). The geometry factories (`ascii_geometry`, `svg_geometry`) are **deferred to Step 4**, where their return types exist.

**Modify `aspuzzle/grids/rectangulargrid.py`** (`RectangularGrid`): implement `neighbor` (pure arithmetic on `row`/`col`, off-grid ⇒ `None`), `corner_names` (`nw/ne/se/sw`), `corner_across` (crossing an incident edge flips that axis of the corner name), `all_cells` (row-major).
...
- **The neighbor-vs-ASP conformance test:** solve a throwaway `Puzzle` holding a 3×3 `RectangularGrid` with `OrthogonalDir` shown, collect its atoms (direction-labeled, so per-direction mapping is checked too), diff against `{(c, d, neighbor(c, d))}` from Python.
...
- `aspuzzle/rendering/ascii/geometry.py` — `AsciiGeometry` Protocol (re-exports `LayoutNeeds`).
...
**Modify `aspuzzle/rendering/scene.py`:** the substrate vocabulary (design §3.7, post-review): `Lattice` (NONE/FRAME/FULL), `SceneStyle.frame_weight`/`vertex_dots`, `Scene.backend_styles` + `style_for(backend)` (whole-style replacement; `layout_needs(backend)` consults it).

**Modify `aspuzzle/rendering/grids/rectangular_ascii.py`:** layout selection (compact iff the backend's `style_for` has `Lattice.NONE`, no vertex dots, and `needs` has no edges/vertices/`label_margins`), interleaved edge lanes with **collapsing**, direction-flag stamping for `EdgeSegment`/frame, `resolve_junctions` via the `frozenset[str]` table extended with the double-line family (`═║╔…` for HEAVY, mixed weight = heavy wins), `VertexMark` placement, substrate painting (`Lattice.FULL` wireframe with light lanes; `vertex_dots` at lattice intersections), `OutsideLabel` margins (`("s", c)` above column c; `("e", r)` left of row r). Stacked `offset` rings deferred (see the design's §11 entry): the geometry raises loudly on `offset > 0` until a solver needs a second ring.
...
- Label margins: one ring with max-width reservation; nonzero `offset` pinned as a loud raise (the ring deferral).
...
**Create:**
- `aspuzzle/rendering/spec.py` — `RenderContext`, `Colorer`, `GlyphRule`, `FillRule`, `PathRule`, `EdgeRule`, `LinkRule`, `FromPredicate`, `FromClues`, `RegionFillRule`, `RegionBorderRule`, `RegionBoundaryRule`, `CustomRule`, `AtomRule`, `LineLabels`, `RenderSpec` (incl. `style` + per-backend `backend_styles`, design §3.7), and the free function `build_scene(grid, spec, grid_data, solution)` — clue styles first (paint-below, matching `_build_render_grid`'s order), then atom rules (data-driven rules run with `solution=None`), then labels; provenance and `backends` stamped mechanically; **eager field-name validation with precise errors**.
- `aspuzzle/rendering/regioncolor.py` — **pure deterministic Python** replaces the ASP colorer (decision July 2026: clingo's first model is stable per-binary but not contractually stable across versions/platforms, and CI must not flap): adjacency computed via `grid.neighbor`, then first-fit backtracking over a fixed most-constrained-first ordering, colors in palette order. Identical results across runs, platforms, and library versions; ~100 pairwise-adjacent regions color in ~5ms. Needs only the `RenderGrid` surface — no shadow puzzle, no solve. Plus `DEFAULT_REGION_PALETTE` (≥4 `PaletteColor`s). Pinned by unit tests (determinism across runs and insertion orders; adjacent-differ). The search is iterative (no recursion limit at large region counts) and step-budgeted: disconnected regions sharing an id (repeated clue values) make the adjacency graph non-planar, so exhaustive search is bounded and falls back to a deterministic greedy best-effort, with an honest error when a coloring provably does not exist. `grids/region_coloring.py` (the old ASP version) is left untouched — old-path Galaxies/Stitches/Starbattle still import it — until Step 11 deletes it.
...
**Create `tests/rendering/test_solver_specs.py`** (grows each wave): Tents spec is pure data — two `LineLabels`, one `GlyphRule`, one SVG-only `LinkRule` for the tree-to-tent ties (TieDestination flipped to shown; expected solutions regenerated); preview scene contains `GIVEN` tree glyphs + labels and zero `DERIVED` elements; solved scene from a hand-written `{"tent": [...]}` dict contains yellow "A" glyphs. No clingo in the loop.
...
| **Skyscrapers** | 8 | letter-map duplicate → `glyph_for_value`; `height` → `GlyphRule(value_field="value")`; **+ `LineLabels` ×4** (`s/n/e/w` per the existing `clue_mapping`), `SceneStyle(lattice=Lattice.FRAME)` per §7.5 | Flagged: edge clues newly rendered; frame added — reviewed delta in both modes |
| **Sudoku** | 8 | `draw_box/rows_per_box/cols_per_box` → `SceneStyle(lattice=Lattice.FRAME)` + `RegionBorderRule(by=block-index lambda over self.block_rows/block_cols)`; letter-map → `glyph_for_value`; `number` → `GlyphRule(value_field="value", GREEN)` | Flagged: boxes now from lane-collapse + junction resolution — target byte-identical to old `├┼┤` output (pre-verified by Step 5's Sudoku-shaped geometry golden); review any junction diffs char-by-char. Boxes render in preview via `by=` (they already do today) |
...
**Post-Step-11 checkpoint:** re-run the four-lens adversarial audit (correctness with mandatory repros, simplification, design fidelity, forward-looking) over the final state — the first run (July 2026, mid-migration after Wave 1) caught a label-width crash, the striped-fill interaction, glyph_for_value overflow, and colorer scaling limits; the rerun validates the deletion step and the accumulated fixes. **Rerun outcome (July 2026, post-Step-11):** correctness found and fixed two bugs (label pitch collapsing at materialized lanes; solution values ≥ 36 crashing the character grid — now the overflow convention in `_value_glyph`) plus three small holes (`SceneStyle.empty.backends`, `RegionBorderRule` boundary edges around regionless cells, `CellLink` width errors); topology canonicalization, the colorer, junction resolution, and backend filtering were brute-forced/fuzzed clean. Simplification and design-fidelity confirmed the final state faithful, with all divergences resolved doc-side (this document and the design amended accordingly); the forward-looking lens's readiness findings are recorded in the design's §6/§6.1/§11 and the follow-on entries below.

## Step 12 — Signature-keyed solution dicts (done July 2026)

Solution dicts key by full signature: `Solver.solve` buckets on `"name/arity"` (the ASP-standard spelling, matching `#show p/2.`), removing the ambiguity bare names had for same-name/different-arity predicates. Classical negation is handled by **blanket drop**: `-p` atoms never enter solution dicts — no renderer can draw a negated atom meaningfully, and a solver that wants `-p` visible derives a positive alias in one ASP line, so supporting signs in the data format would buy nothing. The drop is silent by design and documented on `solve()`.

String references were **kept, not retired** (SymbolSet/RegionConstructor predicate classes are `construct_puzzle` locals and `get_render_spec` runs pre-construction, so class-only rules would force stash-on-self across ~8 solvers), but they must spell the full signature: `FillRule("wall/1")`. Bare names are rejected at spec construction with a precise error — a typo or arity mismatch can no longer silently match nothing. Class references derive their signature automatically (`get_name()`/`get_arity()`) and keep the isinstance filter as belt-and-braces.

The data-format change ran as one batch: `solve()` bucketing, the string-shape check in `_check_ref_fields`, arity added to the 9 in-tree string refs, every `puzzles/*.json` expected-solutions block rekeyed by script (arity read off each key's first canonical atom), and the hand-written solution dicts in the spec tests. The exhaustive-solve validation (`models=0` + exact set equality) proved the rekeyed blocks against real models immediately, and the cross-solver string-ref guardrail now checks signatures, catching arity mismatches as well as typos.
...
- **SVG backend (chosen as the next milestone, July 2026):** `aspuzzle/rendering/svg/renderer.py` (`SvgRenderer`, `SvgTheme`) against the protocol frozen in Step 4; `svg_geometry()` overrides per grid (~30 lines each); one `--svg out.svg` flag in `solveit.py` calling `SvgRenderer().render(solver.build_scene(sol))` — previews included for free; `tests/goldens/svg/**` joins the golden harness. **Zero solver edits**, enforced by the Step 4 import-boundary tests. Settle the two decisions recorded in the design's §6 at implementation (out-of-drawable pre-filtering via `cell_at` — recommended; whether `bounds` stays on the geometry or the renderer folds it from requested points). **Per-atom path/link colorers ride along**: widen `PathRule.color`/`LinkRule` to accept `Colorer`, with Numberlink pair-coloring as the showcase — which also needs a solver-side shown predicate carrying the pair identity (`propagated_symbol` is hidden today).

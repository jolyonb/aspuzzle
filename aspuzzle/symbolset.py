...
class SymbolInfo:
    ...
    is_range: bool = False
    pool: Pool | None = None
    value_field: str | None = None  # For range symbols, the name of the value field
...
class SymbolSet(Module):
    ...
    def add_symbol(self, name: str, show: bool = True) -> Self:
        ...
        symbol_pred = Predicate.define(name, {"loc": GridCell}, namespace=self.namespace, show=show)
    ...
    def finalize(self) -> None:
        ...
        for _name, symbol_info in self._symbols.items():
    ...
    def make_contiguous(self, symbol_name: str, anchor_cell: Predicate | None = None) -> Self:
        ...
        self.when(anchor_cell).derive(Connected(**{f: getattr(anchor_cell, f) for f in fields}))

        # Propagate connectivity
        C, C_adj = Variable("C"), Variable("C_adj")
        self.when(
